...
| 路径 | 能力 | 代价 |
|---|---|---|
| **A. 开启 `ERROR_LOG_ENABLED=true`（本次采用）** | 令牌/渠道维度的失败事件 + 错误摘要，请求监控页的失败明细与异常 Key 完整可用 | 需改 systemd 环境变量并重启（约数秒，可回滚） |
| **B. 保持关闭** | 只能用 `perf_metrics.request_count / success_count`（模型×分组，5 分钟桶）做粗粒度成功率 | 无失败明细、无受影响 Key/渠道 |

**本项目策略（已确认）**：部署时开启 `ERROR_LOG_ENABLED=true`（用户已授权，须先备份 unit 文件留回滚点），失败明细走完整路径；代码仍实现"未开启"的降级分支与前端提示，保证配置被回退时页面不会给出假 0。
